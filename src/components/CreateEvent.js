...
const CreateEvent = () => {
    ...
    const handleSubmit = useCallback((eventID,title,date,comment, attendees) => {
        ...
        firestore.collection('events').doc(eventID)
        .set({ eventID:eventID, title:title, date:date, comment:comment, attendees:attendees})
        .then(()=>{
            ...
            <div>
                <form>
                    <DatePicker 
                        ...
                        <br />
                    <select name="title" placeholder="Molten Core / BWL" onChange={event => setTitle(event.target.value)} >
                        <option>Choose dungeon</option>
                        <option>Molten Core</option>
                        <option>Blackwing Lair</option>
                    </select> 
                    <br />
                    <textarea name="comment" placeholder="comment" onBlur={event => setComment(event.target.value)} /> <br />
